...
from .layers.conv_transpose import MakeKerasConv2DTranspose
from .layers.conv1d_transpose import MakeKerasConv1DTranspose
...
mapKerasLayerWithActivation = {
    ...
    "Conv1DTranspose": MakeKerasConv1DTranspose,
    "Conv2DTranspose": MakeKerasConv2DTranspose
}
...
def add_layer_into_RModel(rmodel, layer_data):
    ...
    # These layers require two operators - dense/conv and their activation function
        ...
        if LayerActivation != "linear":
            ...
            if fLayerType == "Conv2D" or fLayerType == "Conv2DTranspose":
                if layer_data["channels_last"]:
                    op = SOFIE.ROperator_Transpose("float")([0, 3, 1, 2], inputs[0], LayerName + "PreTrans")
                    rmodel.AddOperator(move_operator(op))
                    inputs[0] = LayerName + "PreTrans"
            ...
            if fLayerType in ["Conv1D", "Conv2D", "Conv3D", "Conv1DTranspose", "Conv2DTranspose"]:
                if layer_data.get("channels_last", False):
                    # For Conv1D, transpose is [0, 2, 1]
                    # For Conv2D, transpose is [0, 3, 1, 2]
                    # For Conv3D, transpose is [0, 4, 1, 2, 3]
                    perm = [0, 2, 1] if "1D" in fLayerType else ([0, 3, 1, 2] if "2D" in fLayerType else [0, 4, 1, 2, 3])
                    inv_perm = [0, 2, 1] if "1D" in fLayerType else ([0, 2, 3, 1] if "2D" in fLayerType else [0, 2, 3, 4, 1])
                    
                    op = SOFIE.ROperator_Transpose("float")(
                        inv_perm, LayerName + fLayerType, LayerName + "PostTrans"
                    )
                    rmodel.AddOperator(move_operator(op))
...
class PyKeras:
    def Parse(filename, batch_size=1):  # If a model does not have a defined batch size, then assuming it is 1
        ...
        for layer in keras_model.layers:
            ...
            if layer_data["layerType"] in ["Conv1D", "Conv2D", "Conv1DTranspose", "Conv2DTranspose", "MaxPooling2D", "AveragePooling2D", "GlobalAveragePooling2D"]:
